...
- "Start onboarding"
- "Install the 40 active skills"

**Your task:** Install all 40 active skills using the 5-Wave Parallel Orchestration below.
...
Tell user: "Onboarding triggered. I will install 40 active skills using [5-Wave Parallel / Sequential] strategy. Checking for existing installations..."
...
- After all 40 active skills: final complete index
...
5. Report progress: "[X] of 40 active skills installed. Currently on: [skill name]."

DO NOT REMOVE this block until all 40 active skills are installed or attempted.
```

**Step 3: After onboarding completes, clean up**

When all 40 active skills are installed (or attempted), the agent MUST:
...
**The onboarding process must run continuously until all 40 active skills are installed.** Some models will stall, pause, or silently stop mid-installation. This protocol prevents that.
...
"Onboarding progress: [X] of 40 active skills installed. Currently on: [skill name]. [Y] skills remaining."

### Completion

When all 40 active skills are installed:
1. Count and verify: all 40 active skill folders should exist in ~/.openclaw/skills/ (33 and 34 are archived)
...
5. If a gateway restart is needed for any changes to take effect, tell the user:
   "All 40 active skills are installed. To activate the changes, please type /restart in Telegram."
...
**ZERO TOLERANCE: The onboarding does not end until all 40 active skills have been attempted. If you stop early, you failed.**
...
| **Final** | After ALL 40 active skills complete | Complete system index |
...
5. **Wave 5** (Memory): Dispatch sub-agents to verify skill 15, install skill 31, and finalize the memory stack
6. **Wave 6** (Extensions & domain verticals): Dispatch parallel sub-agents for skills 32, 35, 36, 37, 38, 39, 40, 41, **42 (Personal Assistant Library)**, and **43 (Graphify Knowledge Graph)**. Skill 36 (GHL MCP) before Skill 35 (Social Media Planner); Skill 42 requires Skill 23 (installed in Wave 3) and recommends Skill 22 (Wave 3); Skill 43 (Graphify) recommends Skill 23 (Wave 3) so there is a built workforce to map, installs the FREE AST hook + maps ONCE on the client's OWN model (never operator keys, N29). These ship via the generic `[0-9]*/` copy loop, so the agent installs/activates each per its INSTALL.md after the core system is up.
...
4. **Write ONBOARDING COMPLETE to MEMORY.md**
   - Add entry: "OpenClaw onboarding completed on [date]. All 40 active skills installed."
...
If `sessions_spawn` doesn't work, install 40 active skills one at a time:
...
The OpenClaw onboarding uses a **5-WAVE PARALLEL STRATEGY** to install 40 active skills efficiently.
...
│         │                 │ Memory stack verification                         │
│         │                 │ NOTE: Skills 33-34 archived (merged into 23)    │
├─────────┼─────────────────┼─────────────────────────────────────────────────┤
│ Wave 6  │ Parallel        │ Agent G: 32, 36, 35  (36 before 35)               │
│         │ (domain-aware)  │ Agent H: 37, 38                                  │
│         │                 │ Agent I: 39, 40, 41                              │
│         │                 │ Agent J: 42 Personal Assistant Library            │
│         │                 │ (42 requires 23; recommends 22)                  │
...
#### Spawn Wave 6 - Extensions & domain verticals (Skills 32, 35-42)
```bash
# 36 (GHL MCP) must finish before 35 (Social Media Planner). 42 requires 23 (Wave 3) + recommends 22 (Wave 3).
for skill in "32-command-center-setup" "36-ghl-mcp-setup"; do
  num=${skill%%-*}
  openclaw agent spawn \
    --task "Install skill $num from ~/.openclaw/onboarding/$skill. Follow Teach Yourself Protocol. Report completion status." \
    --label "skill-$num" &
...
wait
for skill in "35-social-media-planner" "37-zhc-closeout" "38-conversational-ai-system" "39-real-estate-playbook" "40-zhc-public-records-scraper" "41-build-with-ai-playbook" "42-personal-assistant-library"; do
  num=${skill%%-*}
  openclaw agent spawn \
    --task "Install skill $num from ~/.openclaw/onboarding/$skill. Follow Teach Yourself Protocol. Report completion status." \
    --label "skill-$num" &
...
"Onboarding in progress: [X] of 40 active skills complete. Currently on: [skill name]."

---

## FINAL STEP - SET UP WEEKLY AUTO-UPDATE (Agent Runs This)

After all 40 active skills are installed, run this as the final step.
...
After all 40 active skills are installed, run the final Gemini Engine indexing:
...
Then write to MEMORY.md: "ONBOARDING COMPLETE - [date] - All 40 active skills processed"
