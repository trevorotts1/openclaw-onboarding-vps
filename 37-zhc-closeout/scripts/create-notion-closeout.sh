...
# ---- resolve parent page ----
#
# v10.X.4 fallback chain (Evelyn 2026-05-26 postmortem):
#   1. NOTION_CLOSEOUT_PARENT_PAGE_ID env (operator override)
#   2. Search for "BlackCEO" parent page
#   3. Search for "OpenClaw" parent page
#   4. Search for any page titled "Your Zero-Human Company" or
#      "Zero Human Company" left over from a prior run
#   5. Create at workspace root (no parent) -- Notion API supports root pages
...
if [[ -z "$PARENT_PAGE_ID" ]]; then
  log "WARN" "fallback 5: no parent page found -- will create at WORKSPACE ROOT (no parent)"
  log "WARN" "fallback 5: this only works if the Notion integration is granted access to a root-level page; otherwise the create call will 401"
  PARENT_PAGE_ID=""
...
if [[ "$PARENT_KIND" == "workspace-root" ]]; then
  # v10.X.4: no parent page available; create at workspace root. Notion API
  # accepts parent.type=workspace, but the integration must have workspace
  # access. If this returns 401/403 it means the integration is page-scoped --
  # operator must set NOTION_CLOSEOUT_PARENT_PAGE_ID and re-run.
  root_body=$(jq -n \
    ...
    '{
      ...
      children: [
        ...
        {object: "block", type: "paragraph", paragraph: {rich_text: [{type:"text", text:{content: "This is your full closeout doc. Read it in order. The 9 sections below explain what you have, how it works, and how to use it."}}]}}
      ]
    }')
else
  root_body=$(jq -n \
