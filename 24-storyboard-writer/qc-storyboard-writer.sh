#!/usr/bin/env bash
# Skill 24 — Storyboard Writer — Install QC
set -u
...
echo ""
echo "═══ Skill 24 — Storyboard Writer — Install QC ═══"
echo ""
assert "Skill 24 folder present" "[ -d \"$SKILLS_DIR_DEFAULT/24-storyboard-writer\" ]"
assert "Python 3 installed" "command -v python3"
warn_only "Storyboard output folder writable" "mkdir -p \"$WORKSPACE/storyboards\" 2>/dev/null && [ -w \"$WORKSPACE/storyboards\" ]"
echo ""
echo "═══ Result: $PASS passed | $FAIL failed | $WARN warnings ═══"
[ $FAIL -gt 0 ] && { red "Skill 24 QC FAILED"; exit 1; } || { green "Skill 24 QC PASS"; exit 0; }
