...
```bash
SKILL_DIR="/data/.openclaw/master-files/OpenClaw Onboarding/10-github-setup"
ls -1 "$SKILL_DIR"
...
```bash
grep -n '^GITHUB_TOKEN=' /data/.openclaw/secrets/.env 2>/dev/null
grep -n '^GITHUB_USERNAME=' /data/.openclaw/secrets/.env 2>/dev/null
```
