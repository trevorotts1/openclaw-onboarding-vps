...
| 40-zhc-public-records-scraper | **ZHC Public Records Scraper (v1.0.0)** — tiered public-records engine: **Tier 1** 20 curated major-county portals (Cook IL, LA, Maricopa AZ, Harris TX, San Diego, Orange CA, …); **Tier 2** platform-adapter framework (1 adapter/vendor — Tyler + GovOS examples); **Tier 3** operator-buildable validated scraper configs; **Tier 4** HONEST GAP (no source/blocked → told to operator, never fabricated). Auto-detect address/ZIP → county+state → tier. Respects robots.txt + ToS, attributes source + timestamp, per-day/per-target rate limits + daily cost cap + cost estimate before bulk ops, 30-day cache. Pairs w/ skill 39 (pre-foreclosure/NOD/tax-delinquency). Emits `<MASTER_FILES_DIR>/public-records-queries.jsonl` (F52 contract). 6 scripts + 2 adapters + 3 protocols + 4 references. `curl` required (runtime). |
