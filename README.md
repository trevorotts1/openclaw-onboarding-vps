...
> **Version:** see `/version` - this repo at v10.16.32.
...
>
> **NOTE (v10.16.32) — fleet standards: sub-agent permissions + 50MB media cap + version-marker reconciliation.** Ships `apply-fleet-standards.sh` (sub-agents fully permitted + Telegram media cap 50 MB) wired into install/update; reconciles all 9 version markers to v10.16.32 (update-skills.sh had already advanced to this marker on client machines). CI (`version-consistency.yml`) proves all nine agree.
>
...
## 🔴 READ THIS FIRST — Deployment Models & Install Path (v10.16.32)
...
**Current Version:** see `/version` (currently **v10.16.32**) — see [CHANGELOG.md](CHANGELOG.md) for the full release history.
