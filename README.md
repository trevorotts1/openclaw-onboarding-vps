...
> **Version:** see `/version` - this repo at v10.16.34.
...
>
> **NOTE (v10.16.34) — ECHO-BACK GATE: Rule 0 of Big Project Mode.** Adds RULE 0 (THE ECHO-BACK GATE) to `BIG-PROJECT-MODE.md` as Rule 0 before Rule 1 (eight rules → nine rules). The orchestrator must echo back all rules in its own words + the full work-slice list + exact model strings, and wait for GO before spawning anything. `apply-fleet-standards.sh` idempotency upgraded to v2 heading (`## BIG PROJECT MODE (v2)`) — existing clients with the v1 block are upgraded automatically on next run; second run is a no-op. Per-release version history lives in [CHANGELOG.md](CHANGELOG.md). CI (`version-consistency.yml`) proves all nine version markers agree.
>
...
## 🔴 READ THIS FIRST — Deployment Models & Install Path (v10.16.34)
...
**Current Version:** see `/version` (currently **v10.16.34**) — see [CHANGELOG.md](CHANGELOG.md) for the full release history.
