...
> **Version:** see `/version` - this repo at v10.16.33.
...
>
> **NOTE (v10.16.33) — BIG PROJECT MODE: universal AGENTS.md standard.** Ships `BIG-PROJECT-MODE.md` (the client-universal large-build standard: orchestrator pastes the shared document byte-identical atop every worker, warm-up then fan-out, skinny orchestrator, independent numeric QC) and extends `apply-fleet-standards.sh` to idempotently append a compact `## BIG PROJECT MODE` section to every agent's active-workspace `AGENTS.md` (resolved to `/data/.openclaw/workspace` on the VPS layout) on install AND update. Per-release version history lives in [CHANGELOG.md](CHANGELOG.md). CI (`version-consistency.yml`) proves all nine version markers agree.
>
...
## 🔴 READ THIS FIRST — Deployment Models & Install Path (v10.16.33)
...
**Current Version:** see `/version` (currently **v10.16.33**) — see [CHANGELOG.md](CHANGELOG.md) for the full release history.
