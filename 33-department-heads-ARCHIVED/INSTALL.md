...
| OpenClaw config | `ls /data/.openclaw/openclaw.json` | File exists |
| /data/.openclaw/workspace directory | `ls /data/.openclaw/workspace/` | Directory exists |
...
Creates `/data/.openclaw/workspace/departments/` if it does not exist.

### Step 2: Create 17 Department Subdirectories with SOUL.md
For each of the 17 departments, the script:
- Creates a subdirectory under `/data/.openclaw/workspace/departments/` (e.g., `/data/.openclaw/workspace/departments/marketing/`)
...
- Reads `/data/.openclaw/openclaw.json`
...
- `workspace` - Path to department folder (e.g., `/data/.openclaw/workspace/departments/marketing`)
...
```bash
ls /data/.openclaw/workspace/departments/
```
...
```bash
for d in /data/.openclaw/workspace/departments/*/; do
  echo -n "$d: "
...
**Want to start fresh?**
```bash
rm -rf /data/.openclaw/workspace/departments/
# Then re-run install.sh
```
Note: You will also need to manually remove the agent entries from `/data/.openclaw/openclaw.json` if you want a clean slate.
