...
| **Persona selector (`persona-selector-v2.py`)** | Get the governing persona for the SOP-authoring task so the procedure carries the right methodology | `scripts/persona-selector-v2.py --task "..." --department Openclaw Maintenance` | The persona governs HOW you structure the steps and what quality bar you hold. |
...
2. **Get the governing persona.** Run `persona-selector-v2.py --task "<task>" --department Openclaw Maintenance`. Adopt that persona's methodology for structuring the procedure.
...
- **Director of Openclaw Maintenance** — gives you: a no-SOP trigger naming the blocked task, the requesting role, and the deadline; frequency: on-demand.
...
- **Director of Openclaw Maintenance** — you give them: closure notice + the closed-gap count; the upstream-candidate flag for universal SOPs.
...
|-----------|---------------|------------------------|-------|
| Task is ambiguous; cannot define "done" | Director of Openclaw Maintenance | Master Orchestrator | Human owner via Telegram |
| API docs unreachable / operation undocumented | Director of Openclaw Maintenance | OpenClaw-Maintenance dept | Human owner (supply credentials/docs) |
| Research cannot establish a confident procedure | Director of Openclaw Maintenance | Deep-Research-Specialist | Human owner |
| SOP keeps failing QC ≥8.5 after 3 surgical loops | Department QC-Specialist | Master Orchestrator | Human owner |
| Task belongs to a different department | Director of Openclaw Maintenance (re-route) | Master Orchestrator | — |
