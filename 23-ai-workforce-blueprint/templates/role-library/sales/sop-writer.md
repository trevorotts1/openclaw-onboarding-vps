...
1. **No-SOP-blocker resolution time**
   ...
   - Reported to: Director of Sales, weekly.
...
| **Persona selector (`persona-selector-v2.py`)** | Get the governing persona for the SOP-authoring task so the procedure carries the right methodology | `scripts/persona-selector-v2.py --task "..." --department Sales` | The persona governs HOW you structure the steps and what quality bar you hold. |
...
2. **Get the governing persona.** Run `persona-selector-v2.py --task "<task>" --department Sales`. Adopt that persona's methodology for structuring the procedure.
...
- **Director of Sales** — gives you: a no-SOP trigger naming the blocked task, the requesting role, and the deadline; frequency: on-demand.
...
- **Director of Sales** — you give them: closure notice + the closed-gap count; the upstream-candidate flag for universal SOPs.
...
|-----------|---------------|------------------------|-------|
| Task is ambiguous; cannot define "done" | Director of Sales | Master Orchestrator | Human owner via Telegram |
| API docs unreachable / operation undocumented | Director of Sales | OpenClaw-Maintenance dept | Human owner (supply credentials/docs) |
| Research cannot establish a confident procedure | Director of Sales | Deep-Research-Specialist | Human owner |
| SOP keeps failing QC ≥8.5 after 3 surgical loops | Department QC-Specialist | Master Orchestrator | Human owner |
| Task belongs to a different department | Director of Sales (re-route) | Master Orchestrator | — |
