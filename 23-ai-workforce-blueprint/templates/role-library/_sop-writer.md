...
- **{{DIRECTOR_OR_MASTER_ORCHESTRATOR}}** — gives you: a no-SOP trigger naming the blocked task, the requesting role, and the deadline; frequency: on-demand.
...
- **{{DIRECTOR_OR_MASTER_ORCHESTRATOR}}** — you give them: closure notice + the closed-gap count; the upstream-candidate flag for universal SOPs.
...
|-----------|---------------|------------------------|-------|
| Task is ambiguous; cannot define "done" | {{DIRECTOR_OR_MASTER_ORCHESTRATOR}} | Master Orchestrator | Human owner via Telegram |
| API docs unreachable / operation undocumented | {{DIRECTOR_OR_MASTER_ORCHESTRATOR}} | OpenClaw-Maintenance dept | Human owner (supply credentials/docs) |
| Research cannot establish a confident procedure | {{DIRECTOR_OR_MASTER_ORCHESTRATOR}} | Deep-Research-Specialist | Human owner |
| SOP keeps failing QC ≥8.5 after 3 surgical loops | Department QC-Specialist | Master Orchestrator | Human owner |
| Task belongs to a different department | {{DIRECTOR_OR_MASTER_ORCHESTRATOR}} (re-route) | Master Orchestrator | — |
