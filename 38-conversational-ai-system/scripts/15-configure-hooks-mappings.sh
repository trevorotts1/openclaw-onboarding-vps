...
if [[ "$HAS_MAPPING" == "true" ]]; then
  ...
  NEW_MAPPING="$(jq -n \
    ...
    '{
      ...
      messageTemplate:"INBOUND MESSAGE FROM GOHIGHLEVEL — {{channel}} channel From: {{first_name}} {{last_name}} Phone: {{phone}} Email: {{email}} Contact ID: {{contact_id}} Location ID: {{location_id}} Location name: {{location_name}} Customer message subject: {{subject}} Customer message body: {{message_body}} MEMORY — READ FIRST (this is a SINGLE-TURN hook session; your only memory of this contact is the conversation log file): BEFORE drafting anything, read this contact'\''s conversation log at <MASTER_FILES_DIR>/conversational-logs/{{contact_id}}__<name>.md for the full prior conversation and any in-progress booking/topic; if the file is missing, treat this as a new contact. To pull prior GHL thread history when the local log is thin, GET conversations/search?locationId={{location_id}}&contactId={{contact_id}} to find the thread, then GET conversations/<conversationId>/messages to read it (conversationId is the READ key only — it is NEVER a send-body field). CONTINUE: reply continuing any in-progress topic/booking found in the log — do not restart or re-ask what the log already answered. MANDATORY — SEND on the SAME channel the message arrived on, do not just draft: read the inbound channel ({{channel}}) and SEND via the GHL Conversations API — POST conversations/messages — with type = the MIRRORED channel value (SMS->SMS, Email->Email, Facebook->FB, Instagram->IG, WhatsApp->WhatsApp, Live Chat->Live_Chat); do NOT hardcode SMS. Send body = {type:<mirrored>, contactId:{{contact_id}}, locationId:{{location_id}}, message:<your reply>} (Email also needs subject+html+emailFrom+emailTo); GHL threads it into this contact'\''s conversation BY contactId and returns conversationId+messageId. Use your installed GHL skill / the shapes in TOOLS.md. Composing or drafting a reply is NOT sending — the customer receives nothing unless you make the API call. Do NOT end your turn until the send call returns a messageId. (GMB inbound cannot be replied to via this API — escalate per protocol.) APPEND — LOG AFTER SENDING: append this inbound message AND your sent reply to <MASTER_FILES_DIR>/conversational-logs/{{contact_id}}__<name>.md (create the file if missing) per the conversation-log protocol — a reply that fails to update the log is a failure (see AGENTS.md for the full conversation-memory protocol).",
...
PAYLOAD="{\"id\":\"${ROUTE_ID}\",\"match\":\"${ROUTE_ID}\",\"action\":\"agent\",\"agent_id\":\"${ROUTING_AGENT_ID:-main}\",\"model\":\"ollama/deepseek-v4-flash:cloud\",\"wakeMode\":\"now\",\"name\":\"GHL Sales Inbound\",\"session_key\":\"hook:ghl:sms:e2e-test-001\",\"messageTemplate\":\"Respond as the Sales agent. MANDATORY — SEND on the SAME channel the message arrived on, do not just draft: SEND via the GHL Conversations API (POST conversations/messages) with type = the MIRRORED channel value, contactId, locationId, and message — GHL threads it by contactId. Composing or drafting a reply is NOT sending — the customer receives nothing unless you make the API call. Do NOT end your turn until the send call returns a messageId.\",\"deliver\":false,\"timeoutSeconds\":300,\"channel\":\"sms\",\"to\":\"+15555550100\",\"thinking\":\"medium\",\"contact_id\":\"e2e-test-001\",\"first_name\":\"E2E\",\"last_name\":\"Test\",\"email\":\"e2e@example.com\",\"phone\":\"+15555550100\",\"subject\":\"\",\"message_body\":\"End-to-end setup verification.\",\"location_id\":\"e2e-loc-001\",\"location_name\":\"E2E Test Location\"}"
